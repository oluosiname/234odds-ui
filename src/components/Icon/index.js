import React from 'react';
import Icon from './Icon';

export const Betking = props => <Icon {...props} name="betking" />;
export const IconAll = props => <Icon {...props} name="icon-all" />;
export const IconBack = props => <Icon {...props} name="icon-back" />;
export const IconCalendar = props => <Icon {...props} name="icon-calendar" />;
export const IconCalendars = props => <Icon {...props} name="icon-calendars" />;
export const IconCancel = props => <Icon {...props} name="icon-cancel" />;
export const IconClose = props => <Icon {...props} name="icon-close" />;
export const IconClosee = props => <Icon {...props} name="icon-closee" />;
export const IconMenu = props => <Icon {...props} name="icon-menu" />;
export const IconMenus = props => <Icon {...props} name="icon-menus" />;
export const IconSearch = props => <Icon {...props} name="icon-search" />;
export const IconToday = props => <Icon {...props} name="icon-today" />;
export const IconUndo = props => <Icon {...props} name="icon-undo" />;
export const Merrybet = props => <Icon {...props} name="merrybet" />;