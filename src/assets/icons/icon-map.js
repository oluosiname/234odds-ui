import { ReactComponent as Betking } from '../images/betking.svg';
import { ReactComponent as IconArsenal } from '../images/icon-arsenal.svg';
import { ReactComponent as IconAsMonaco } from '../images/icon-as-monaco.svg';
import { ReactComponent as IconAstonVilla } from '../images/icon-aston-villa.svg';
import { ReactComponent as IconBarnsley } from '../images/icon-barnsley.svg';
import { ReactComponent as IconBetJa } from '../images/icon-bet9ja.svg';
import { ReactComponent as IconBetking } from '../images/icon-betking.svg';
import { ReactComponent as IconBetnaija } from '../images/icon-betnaija.svg';
import { ReactComponent as IconBirmingham } from '../images/icon-birmingham.svg';
import { ReactComponent as IconBordeaux } from '../images/icon-bordeaux.svg';
import { ReactComponent as IconBournemouth } from '../images/icon-bournemouth.svg';
import { ReactComponent as IconBrighton } from '../images/icon-brighton.svg';
import { ReactComponent as IconBundesliga } from '../images/icon-bundesliga.svg';
import { ReactComponent as IconBurnley } from '../images/icon-burnley.svg';
import { ReactComponent as IconCalenda } from '../images/icon-calenda.svg';
import { ReactComponent as IconCalendar } from '../images/icon-calendar.svg';
import { ReactComponent as IconChampionsLeague } from '../images/icon-champions-league.svg';
import { ReactComponent as IconChelsea } from '../images/icon-chelsea.svg';
import { ReactComponent as IconCrystalPalace } from '../images/icon-crystal-palace.svg';
import { ReactComponent as IconDijonFco } from '../images/icon-dijon-fco.svg';
import { ReactComponent as IconEngland } from '../images/icon-england.svg';
import { ReactComponent as IconEuropaLeague } from '../images/icon-europa-league.svg';
import { ReactComponent as IconEverton } from '../images/icon-everton.svg';
import { ReactComponent as IconFcMetz } from '../images/icon-fc-metz.svg';
import { ReactComponent as IconFulham } from '../images/icon-fulham.svg';
import { ReactComponent as IconLaLiga } from '../images/icon-la-liga.svg';
import { ReactComponent as IconLeeds } from '../images/icon-leeds.svg';
import { ReactComponent as IconLeicester } from '../images/icon-leicester.svg';
import { ReactComponent as IconLigue } from '../images/icon-ligue-1.svg';
import { ReactComponent as IconLiverpool } from '../images/icon-liverpool.svg';
import { ReactComponent as IconLorient } from '../images/icon-lorient.svg';
import { ReactComponent as IconLoscLille } from '../images/icon-losc-lille.svg';
import { ReactComponent as IconManchesterCity } from '../images/icon-manchester-city.svg';
import { ReactComponent as IconManchesterUnited } from '../images/icon-manchester-united.svg';
import { ReactComponent as IconMerrybet } from '../images/icon-merrybet.svg';
import { ReactComponent as IconMillwall } from '../images/icon-millwall.svg';
import { ReactComponent as IconMontpellierHsc } from '../images/icon-montpellier-hsc.svg';
import { ReactComponent as IconNaijabet } from '../images/icon-naijabet.svg';
import { ReactComponent as IconNairabet } from '../images/icon-nairabet.svg';
import { ReactComponent as IconNantes } from '../images/icon-nantes.svg';
import { ReactComponent as IconNewcastle } from '../images/icon-newcastle.svg';
import { ReactComponent as IconNorwich } from '../images/icon-norwich.svg';
import { ReactComponent as IconOgcNice } from '../images/icon-ogc-nice.svg';
import { ReactComponent as IconOlympiqueLyonnais } from '../images/icon-olympique-lyonnais.svg';
import { ReactComponent as IconOlympiqueMarseille } from '../images/icon-olympique-marseille.svg';
import { ReactComponent as IconOlympiqueNimes } from '../images/icon-olympique-nimes.svg';
import { ReactComponent as IconPremierLeague } from '../images/icon-premier-league.svg';
import { ReactComponent as IconPsg } from '../images/icon-psg.svg';
import { ReactComponent as IconRcLens } from '../images/icon-rc-lens.svg';
import { ReactComponent as IconReset } from '../images/icon-reset.svg';
import { ReactComponent as IconSaintEtienne } from '../images/icon-saint-etienne.svg';
import { ReactComponent as IconScoAngers } from '../images/icon-sco-angers.svg';
import { ReactComponent as IconSerieA } from '../images/icon-serie-a.svg';
import { ReactComponent as IconSheffieldUnited } from '../images/icon-sheffield-united.svg';
import { ReactComponent as IconSouthampton } from '../images/icon-southampton.svg';
import { ReactComponent as IconSportybet } from '../images/icon-sportybet.svg';
import { ReactComponent as IconStadeBrestois } from '../images/icon-stade-brestois.svg';
import { ReactComponent as IconStadeReims } from '../images/icon-stade-reims.svg';
import { ReactComponent as IconStadeRennes } from '../images/icon-stade-rennes.svg';
import { ReactComponent as IconStrasbourgAlsace } from '../images/icon-strasbourg-alsace.svg';
import { ReactComponent as IconToda } from '../images/icon-toda.svg';
import { ReactComponent as IconToday } from '../images/icon-today.svg';
import { ReactComponent as IconTodays } from '../images/icon-todays.svg';
import { ReactComponent as IconTottenham } from '../images/icon-tottenham.svg';
import { ReactComponent as IconUndo } from '../images/icon-undo.svg';
import { ReactComponent as IconWatford } from '../images/icon-watford.svg';
import { ReactComponent as IconWestBrom } from '../images/icon-west-brom.svg';
import { ReactComponent as IconWestHam } from '../images/icon-west-ham.svg';
import { ReactComponent as IconWigan } from '../images/icon-wigan.svg';
import { ReactComponent as IconWolves } from '../images/icon-wolves.svg';
import { ReactComponent as IconWorld } from '../images/icon-world.svg';
import { ReactComponent as IconWycombe } from '../images/icon-wycombe.svg';
import { ReactComponent as Merrybet } from '../images/merrybet.svg';

export default {
"betking": Betking, 
"icon-arsenal": IconArsenal, 
"icon-as-monaco": IconAsMonaco, 
"icon-aston-villa": IconAstonVilla, 
"icon-barnsley": IconBarnsley, 
"icon-bet9ja": IconBetJa, 
"icon-betking": IconBetking, 
"icon-betnaija": IconBetnaija, 
"icon-birmingham": IconBirmingham, 
"icon-bordeaux": IconBordeaux, 
"icon-bournemouth": IconBournemouth, 
"icon-brighton": IconBrighton, 
"icon-bundesliga": IconBundesliga, 
"icon-burnley": IconBurnley, 
"icon-calenda": IconCalenda, 
"icon-calendar": IconCalendar, 
"icon-champions-league": IconChampionsLeague, 
"icon-chelsea": IconChelsea, 
"icon-crystal-palace": IconCrystalPalace, 
"icon-dijon-fco": IconDijonFco, 
"icon-england": IconEngland, 
"icon-europa-league": IconEuropaLeague, 
"icon-everton": IconEverton, 
"icon-fc-metz": IconFcMetz, 
"icon-fulham": IconFulham, 
"icon-la-liga": IconLaLiga, 
"icon-leeds": IconLeeds, 
"icon-leicester": IconLeicester, 
"icon-ligue-1": IconLigue, 
"icon-liverpool": IconLiverpool, 
"icon-lorient": IconLorient, 
"icon-losc-lille": IconLoscLille, 
"icon-manchester-city": IconManchesterCity, 
"icon-manchester-united": IconManchesterUnited, 
"icon-merrybet": IconMerrybet, 
"icon-millwall": IconMillwall, 
"icon-montpellier-hsc": IconMontpellierHsc, 
"icon-naijabet": IconNaijabet, 
"icon-nairabet": IconNairabet, 
"icon-nantes": IconNantes, 
"icon-newcastle": IconNewcastle, 
"icon-norwich": IconNorwich, 
"icon-ogc-nice": IconOgcNice, 
"icon-olympique-lyonnais": IconOlympiqueLyonnais, 
"icon-olympique-marseille": IconOlympiqueMarseille, 
"icon-olympique-nimes": IconOlympiqueNimes, 
"icon-premier-league": IconPremierLeague, 
"icon-psg": IconPsg, 
"icon-rc-lens": IconRcLens, 
"icon-reset": IconReset, 
"icon-saint-etienne": IconSaintEtienne, 
"icon-sco-angers": IconScoAngers, 
"icon-serie-a": IconSerieA, 
"icon-sheffield-united": IconSheffieldUnited, 
"icon-southampton": IconSouthampton, 
"icon-sportybet": IconSportybet, 
"icon-stade-brestois": IconStadeBrestois, 
"icon-stade-reims": IconStadeReims, 
"icon-stade-rennes": IconStadeRennes, 
"icon-strasbourg-alsace": IconStrasbourgAlsace, 
"icon-toda": IconToda, 
"icon-today": IconToday, 
"icon-todays": IconTodays, 
"icon-tottenham": IconTottenham, 
"icon-undo": IconUndo, 
"icon-watford": IconWatford, 
"icon-west-brom": IconWestBrom, 
"icon-west-ham": IconWestHam, 
"icon-wigan": IconWigan, 
"icon-wolves": IconWolves, 
"icon-world": IconWorld, 
"icon-wycombe": IconWycombe, 
"merrybet": Merrybet, 
};